"""Session lifecycle management for McDonald's AI Drive-Thru realtime middleware.
...
_NUDGE_TEXT = (
    ...
    "in your crew member persona, ask whether they need anything else with their order. Do not greet "
...
class SessionManager:
    ...
    def __init__(self, prompt_loader=None, clock: Callable[[], float] | None = None):
        ...
        # Last few guest/crew turns per session, replayed into a resumed upstream.
    ...
    def build_rehydration_item(self, session_id: str) -> str:
        ...
        history = "\n".join(f"{'Guest' if role == 'guest' else 'Crew member'}: {text}" for role, text in turns)
