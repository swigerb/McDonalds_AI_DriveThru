"""Browser websocket transport regressions (ported from the Sonic 2026-09-22 incident).
...
class BrowserSocketTransportTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_busy_rejection_socket_does_not_negotiate_deflate_either(self):
        with patch.object(self.rtmt._sessions, "can_accept_session", return_value=False):
            browser = await self.client.ws_connect("/realtime", compress=15)
        self.assertEqual(browser.compress, 0)
        msg = await asyncio.wait_for(browser.receive(), 5)
        self.assertIn("busy", json.loads(msg.data)["message"])
        await browser.close()
...
class CompressionConfigTests(unittest.TestCase):
    ...
    def test_browser_socket_compression_defaults_off(self):
        ...
        self.assertIs(get_config()["connection"]["ws_compression"], False)

    def test_compression_stays_off_when_config_omits_the_key(self):
        # A trimmed/overridden config.yaml must not silently re-enable deflate.
        import importlib.util

        spec = importlib.util.spec_from_file_location("_rtmt_no_ws_key", rtmt_module.__file__)
        fresh = importlib.util.module_from_spec(spec)
        with patch("config_loader.get_config", return_value={"connection": {}, "security": {}}):
            spec.loader.exec_module(fresh)
        self.assertIs(fresh._WS_COMPRESS, False)
