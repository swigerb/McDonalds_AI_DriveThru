...
APP_PY = REPO / "app" / "backend" / "app.py"
...
class SessionSecretWiringTests(unittest.TestCase):
    ...
    def test_azd_supplied_auth_secret_stays_in_the_secrets_list(self):
        """The path the demo env uses: AZURE_AUTH_CLIENT_SECRET is set, so the
        auth secret travels in `secrets` next to app-session-secret."""
        block = _backend_module_block(MAIN_BICEP.read_text(encoding="utf-8"))
        self.assertRegex(block, r"secrets:\s*union\(enableAuth && !empty\(authClientSecret\) \? \{ 'aad-client-secret': authClientSecret \} : \{\},")
        upsert = UPSERT_BICEP.read_text(encoding="utf-8")
        self.assertIn("!contains(secrets, s.name)", upsert, "a supplied secret must win over the read-back copy")

    def test_create_app_signs_tokens_with_the_shared_secret(self):
        source = APP_PY.read_text(encoding="utf-8")
        body = source[source.index("async def create_app"):]
        self.assertRegex(body, r"app_secret = load_app_secret\(\)")
        self.assertNotIn("os.urandom", body)
