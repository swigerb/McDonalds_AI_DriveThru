...
from test_rate_limit_recovery import _Harness as _RateLimitHarness
...
class GraceHoldEndToEndTests(_ResumeHarness):
    ...
    async def test_upstream_connect_failure_does_not_leak_an_attached_session(self):
        await self.fake_server.close()
        browser = await self.client.ws_connect("/realtime")
        msg = await asyncio.wait_for(browser.receive(), 15)   # Windows retries a refused connect for ~2s
        self.assertEqual((json.loads(msg.data).get("error") or {}).get("code"), "cloud_unreachable")
        await self._until(lambda: self.sm.active_session_count == 0)
        await browser.close()
...
class ResumeProtocolTests(_ResumeHarness):
    ...
    async def test_tool_error_seam_still_works_on_a_resumed_session(self):
        browser, meta, sid = await self._start_fresh()
        await self._drop(browser, sid)
        again = await self._resume(meta["resumeId"])
        await self._until_event(again, "extension.session_resumed")
        calls: list[str] = []

        async def update_order(args, session_id):
            calls.append(session_id)
            if len(calls) == 1:
                raise RuntimeError("menu lookup blew up")
            return ToolResult("ok", ToolResultDirection.TO_SERVER)
        self.rtmt.tools["update_order"] = Tool(target=update_order, schema={"type": "function", "name": "update_order"})
        upstream, log = self.fake.upstreams[-1], self.fake.connections[-1]

        def outputs():
            return [e["item"] for e in log
                    if e["type"] == "conversation.item.create" and e["item"].get("type") == "function_call_output"]
        for n in (1, 2):
            call = {"type": "function_call", "call_id": f"call_{n}", "name": "update_order", "arguments": '{"action": "add"}'}
            await upstream.send_json({"type": "response.output_item.added", "item": {**call, "arguments": ""}})
            await upstream.send_json({"type": "response.output_item.done", "item": call})
            await self._until(lambda n=n: len(outputs()) >= n)
        self.assertIn("update_order tool failed", outputs()[0]["output"])
        self.assertEqual(outputs()[1]["output"], "ok", "the conversation did not survive the tool error")
        self.assertEqual(calls, [sid, sid], "tools ran against the provisional session, not the resumed order")
        self.assertFalse(again.closed)
        self.assertEqual(set(self.sm._session_map.values()), {sid})
        await again.close()

    async def test_rate_limit_recovery_follows_the_resumed_session(self):
        slept: list[float] = []

        async def no_sleep(delay):
            slept.append(delay)
        self.rtmt._rate_limit_sleep = no_sleep
        browser, meta, sid = await self._start_fresh()
        await self._drop(browser, sid)
        again = await self._resume(meta["resumeId"])
        await self._until_event(again, "extension.session_resumed")
        with self.assertLogs("mcdonalds-drive-thru", level="INFO") as logs:
            await self.fake.upstreams[-1].send_json({"type": "error", "error": {
                "type": "rate_limit_exceeded", "code": "rate_limit_exceeded", "message": "Rate limit reached."}})
            await self._until(lambda: slept)
        rate_lines = [line for line in logs.output if "rate" in line.lower() and "limit" in line.lower()]
        self.assertTrue(rate_lines, logs.output)
        self.assertTrue(all(f"session={sid}" in line for line in rate_lines), rate_lines)
        await again.close()
...
class TranscriptBufferTests(unittest.TestCase):
    ...
    def test_ring_buffer_keeps_the_last_n_turns(self):
        ...
        for i in range(10):
            self.sm.record_turn(self.sid, "guest" if i % 2 else "crew", f"turn {i}")
    ...
    def test_rehydration_item_is_one_system_message_with_the_order(self):
        ...
        self.sm.record_turn(self.sid, "crew", "One cheeseburger, anything else?")
        ...
        self.assertIn("Crew member: One cheeseburger, anything else?", text)
...
RATE_LIMIT_ERROR = {"type": "rate_limit_exceeded", "code": "rate_limit_exceeded",
...
class RehydrationAndNudgeTests(_ResumeHarness):
    ...
    async def _converse_then_drop(self):
        browser, meta, sid = await self._start_fresh()          # greeting: crew member says "hi"
    ...
    async def test_upstream_gets_bootstrap_then_rehydration_and_no_greeting(self):
        ...
        self.assertIn("Crew member: hi", text)
        self.assertEqual(len(self._system_texts(upstream)), 1, "exactly one rehydration item")
        self.assertNotIn("response.create", types, "the crew member spoke unprompted after a resume")
    ...
    async def test_nudge_does_not_fire_on_top_of_a_pending_rate_limit_retry(self):
        gate = asyncio.Event()

        async def gated_sleep(delay):
            await gate.wait()
        self.rtmt._rate_limit_sleep = gated_sleep
        meta, sid = await self._converse_then_drop()
        self.sm.nudge_after_seconds = 0.2
        browser, upstream = await self._resume_ok(meta["resumeId"])
        await self.fake.upstreams[-1].send_json({"type": "error", "error": RATE_LIMIT_ERROR})
        await asyncio.sleep(0.6)
        self.assertEqual(self._nudges(upstream), [], "nudged on top of a pending rate-limit retry")
        gate.set()
        await self._until(lambda: "response.create" in [e["type"] for e in upstream])
        await self._response_done(browser)
        await asyncio.sleep(0.3)
        self.assertEqual([e["type"] for e in upstream].count("response.create"), 1,
                         "the retry and the nudge both asked for a response")
        self.assertEqual(self._nudges(upstream), [])
        await browser.close()

    async def test_a_rate_limited_nudge_is_retried_by_the_ladder_not_renudged(self):
        slept: list[float] = []
        ...
        meta, sid = await self._converse_then_drop()
        self.sm.nudge_after_seconds = 0.2
        real_respond = self.fake._respond
        failed = []

        async def respond(ws):
            if not failed:                                  # the nudge's response hits the rate limit
                failed.append(True)
                await ws.send_json({"type": "response.created", "response": {"id": "resp_nudge"}})
                await ws.send_json({"type": "response.done", "response": {
                    "id": "resp_nudge", "status": "failed", "output": [],
                    "status_details": {"type": "failed", "error": RATE_LIMIT_ERROR}}})
                return
            await real_respond(ws)
        self.fake._respond = respond
        browser, upstream = await self._resume_ok(meta["resumeId"])
        await self._until(lambda: [e["type"] for e in upstream].count("response.create") >= 2)
        await self._response_done(browser)
        await asyncio.sleep(0.4)
        self.assertEqual(len(self._nudges(upstream)), 1, "the nudge must fire exactly once")
        self.assertEqual([e["type"] for e in upstream].count("response.create"), 2, "one nudge + one retry")
        self.assertEqual(len(slept), 1)
        await browser.close()

    async def test_a_dropped_socket_takes_its_pending_nudge_with_it(self):
        meta, sid = await self._converse_then_drop()
        self.sm.nudge_after_seconds = 0.3
        browser, upstream = await self._resume_ok(meta["resumeId"])
        with self.assertLogs("mcdonalds-drive-thru", level="INFO") as logs:
            await self._drop(browser, sid)
            await asyncio.sleep(0.7)
        self.assertEqual([line for line in logs.output if "nudges" in line], [], "a nudge outlived its socket")
        self.assertEqual(self._nudges(upstream), [])
...
class ResumeIdNeverLoggedTests(_ResumeHarness):
    ...
    async def test_resume_id_never_appears_in_logs(self):
        capture = _Capture()
        names = ["", "mcdonalds-drive-thru", "mcdonalds-verbose", "aiohttp", "aiohttp.access", "aiohttp.server", "aiohttp.web"]
...
class RateLimitRetryAcrossDetachTests(_RateLimitHarness):
    """McDonald's R1 rate-limit recovery composed with the grace hold: a pending
    retry belongs to the dropped upstream, and a retry is not guest activity."""
    ...
    async def asyncSetUp(self):
        ...
        gated_sleep = self.rtmt._rate_limit_sleep

        async def recording_sleep(delay):
            try:
                await gated_sleep(delay)
            except asyncio.CancelledError:
                self.retry_cancelled.set()
                raise
        self.rtmt._rate_limit_sleep = recording_sleep

    def _sid(self) -> str:
        return next(iter(self.sm._session_map.values()))

    async def test_browser_drop_cancels_a_pending_retry_and_holds_the_order(self):
        await self.connect()
        sid = self._sid()
        self.gate = asyncio.Event()
        self.fake.script = ["fail"]
        await self.fake.respond()
        await self.until(lambda: self.delays)

        await self.browser.close()
        await asyncio.wait_for(self.retry_cancelled.wait(), 5)
        await self.until(lambda: self.sm.is_detached(sid))
        self.assertIn(sid, order_state_singleton.sessions)
        self.gate.set()
        await self.settle(0.2)
        self.assertEqual(self.fake.count("response.create"), 0, "a retry outlived the dropped socket")
    ...
    async def test_a_retry_is_not_guest_activity(self):
        ...
        await self.fake.respond()           # server-side turn, no guest speech
        await self.until(lambda: self.completed())
        self.assertEqual(self.fake.count("response.create"), 1)
        self.assertEqual(self.sm._last_activity[sid], start, "the retry reset the guest idle clock")
