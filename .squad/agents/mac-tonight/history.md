...
- **Order resume port (2026-09-23, feat/order-resume, cloud realtime only):** Ported Sonic's resume onto `RTMiddleTier._websocket_handler`, which sits behind `ProcessorRouter`, so local mode never enters it.
  - Step 2 handshake: the announcement is held until the first frame or 2 s, so each socket gets exactly one of `session_metadata` (+`resumeId`) or `session_resumed`. `resume_rejected` is always followed by metadata. `end_session` closes with 1000 `session_ended`.
  - Step 3: one system rehydration item goes after the bootstrap and before any `response.create`. The greeting is suppressed, and a 30 s nudge fires once.
  - R1 composition: no nudge on top of a pending retry; a rate-limited nudge is retried by the ladder, not re-nudged; the retry is cancelled on detach; `recovery.set_session_id` makes logs follow the resumed id.
  - No ctx_monitor feeding: McD rtmt doesn't feed it for injected text, the greeting included.
  - Voice: `self.voice_choice` is shared on the RTMiddleTier, so the browser re-sends `extension.set_voice` on resume. The e2e proves the resumed upstream is switched back after the shared voice changed.

- **Issue #4 fix — session.updated leaked instructions/tools (2026-09-24, fix/session-updated-leak):** `_process_message_to_client` had a scrub `case "session.created":` that zeroed `instructions`/`tools`/`max_response_output_tokens` and pinned `voice`/`tool_choice`, but no `case` existed for `session.updated` at all — GA's session.updated (which fires on *every* accepted session.update: bootstrap, voice picker, browser handshake, rejection fallback) fell through unmatched and was relayed byte-for-byte, full system prompt and tool schemas included. Audited the GA server events this middle tier relays (`_PASSTHROUGH_SERVER_TYPES` in `audio_pipeline.py` plus every `case` in the match): `session.created` and `session.updated` are the only two events that ever carry a full `session` object, so both are now covered. Factored the five scrub lines into `RTMiddleTier._scrub_session_for_client()` and call it from both cases. Checked `useRealtime.tsx`'s incoming `switch (message.type)` — it has no `case` for `session.created` or `session.updated` at all, so the frontend reads nothing off either event; nothing to preserve beyond what session.created already exposed. Local mode (`local_processor.py`) is a separate code path that never reaches `rtmt.py` and was already scrubbing (echoes back only what the browser itself sent) — unaffected, its 177 tests still pass.
