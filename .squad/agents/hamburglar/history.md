...
- **Issue #4 — session.updated leak test + mutation check (2026-09-24, fix/session-updated-leak):** Added `SessionScrubTests` to `test_session_bootstrap.py`, reusing the existing fake-GA-upstream harness (`FakeGARealtime`/`_RealtimeHarness`). Drives a real browser socket through the real middle tier, collects every server-to-browser frame up to the first `session.updated`, and asserts both `session.created` and `session.updated` carry neither the real system prompt nor tool schemas — while asserting the upstream fake really did receive the real prompt/tools first, so the test can't pass for the wrong reason (nothing sensitive in flight). Confirmed RED before the fix (assertion failed on the literal system prompt string leaking in `session.updated`). After Mac Tonight's fix (shared `_scrub_session_for_client()` helper called from both `session.created` and `session.updated` cases), confirmed GREEN. Mutation check: reverted the `session.updated` case body to a no-op `pass` — test failed again on the same assertion, confirming the test actually exercises the fix and isn't a false positive. Restored the fix. Full suite: 926 passed (baseline 925 + 1), `ruff check .` clean, frontend 122/122 unchanged. Local mode / Azure Speech mode confirmed unaffected (separate code path, doesn't route through `rtmt.py`; their 177 tests pass).
