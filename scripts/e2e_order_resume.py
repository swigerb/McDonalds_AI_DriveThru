"""Real-browser end-to-end check for order resume (cloud realtime path).

Runs the BUILT frontend (app/backend/static) in headless Edge/Chromium against the REAL
middle tier (RTMiddleTier plus the real update_order/get_order/reset_order tools) and a
fake GA realtime upstream (tests/test_order_resume.FakeGAPerConnection). It uses no Azure
services and no network beyond 127.0.0.1.

Scenarios
  1. A 1011 drop mid-conversation:
     - the browser auto-reconnects and gets the same ticket and session back;
     - the new upstream sees bootstrap session.update -> one system rehydration item
       (with the order) -> no greeting;
     - the guest's voice is re-sent; the mic restarts without a tap;
     - one nudge fires after resume.nudge_after_seconds of silence (shortened here);
     - a raising tool on the resumed socket returns an apology to the model, and the
       next tool call still lands on the resumed order.
  2. Same drop, but getUserMedia refuses without a gesture:
     - the app falls back to "tap to continue";
     - the tap restarts the mic on the same session, with no greeting.
  2b. The guest taps while the socket is still reconnecting:
     - the resume frame still goes first, with the queued session.update after it;
     - same session, no greeting.
  3. A page reload in the same tab:
     - the ticket is restored from the resume id in sessionStorage;
     - tap to continue; same session, no greeting.
  4. Idle close 4000: no reconnect and the id is cleared; a tap starts a fresh session.
  4b. "Start a new order": the order is ended on the server with 1000 session_ended; a
      fresh socket opens without resuming, and the ticket is empty.
  5. Strict autoplay policy:
     - the mic still auto-restarts after a drop (the AudioContext created on the first
       tap stays running);
     - after a reload it waits for a tap.
  Throughout: the resume id never appears in a URL or in the server logs.

Not part of the default test run (it needs a browser). To run:
    .\\.venv\\Scripts\\python.exe -m pip install playwright      # dev-only; via your package proxy
    cd app/frontend && npm run build && cd ../..
    .\\.venv\\Scripts\\python.exe scripts/e2e_order_resume.py [--channel msedge|chrome|chromium] [--headed]
`--channel chromium` needs `python -m playwright install chromium`; msedge and chrome use the
installed browser. Exit code 0 means every check passed.
"""
...
MCFLURRY = {"action": "add", "item_name": "McFlurry with OREO Cookies", "size": "regular", "quantity": 1, "price": 4.39}
COKE = {"action": "add", "item_name": "Coca-Cola", "size": "large", "quantity": 1, "price": 2.29}
PIE = {"action": "add", "item_name": "Apple Pie", "size": "standard", "quantity": 1, "price": 1.79}
TICKET = ("Regular McFlurry with OREO Cookies", "Large Coca-Cola")
GUEST_VOICE = "cedar"
SERVER_VOICE = "shimmer"
REHYDRATION_MARKER = "[Connection restored]"
NUDGE_MARKER = "quiet since their connection came back"
GREETING_MARKER = "greeting"
TOOL_ERROR_MARKER = "tool failed and did not run"
...
def unprompted_creates(frames: list[dict]) -> list[int]:
    """response.create frames that don't follow the nudge item (i.e. a greeting)."""
    return [i for i, f in enumerate(frames)
            if f.get("type") == "response.create" and (i == 0 or NUDGE_MARKER not in item_text(frames[i - 1]))]


def voices_sent(frames: list[dict]) -> list[str]:
    return [((f.get("session") or {}).get("audio") or {}).get("output", {}).get("voice")
            for f in frames if f.get("type") == "session.update"]
...
def boom(_args, _session_id):
    raise RuntimeError("e2e: tool exploded")
...
async def start_servers():
    ...
    rtmt = RTMiddleTier(
        ...
        voice_choice=SERVER_VOICE,
    )
    rtmt.system_message = "You are a McDonald's crew member."
    ...
    rtmt.tools["boom"] = Tool(schema={"type": "function", "name": "boom", "parameters": {"type": "object", "properties": {}}},
                              target=boom)
    sm = rtmt._sessions
...
async def new_page(browser, *, voice: str | None = None):
    context = await browser.new_context(permissions=["microphone"], locale="en-US")
    await context.add_init_script(INIT_SCRIPT)
    if voice:
        await context.add_init_script(f"localStorage.setItem('voiceChoice', '{voice}');")
...
async def on_ticket(page, *names: str) -> bool:
    """Exact-text line items (the menu and upsell banners also mention item names)."""
    for name in names:
...
async def shows(page, text: str, timeout: int = 5000) -> bool:
    try:
        await page.get_by_text(text, exact=True).wait_for(timeout=timeout)
        return True
    except Exception:
        return False


async def scenario_resume(c: Checks, browser, url, fake, rtmt, sm, logs: LogCapture):
    print("\n== 1-3: drop 1011 / gesture fallback / reload (same tab) ==", flush=True)
    context, page = await new_page(browser, voice=GUEST_VOICE)
    console: list[str] = []
    ...
    first_id = await page.evaluate("sessionStorage.getItem('mcdonalds.resumeId')")
    c.check("metadata resumeId stored in sessionStorage", bool(first_id) and len(first_id) >= 40)
    c.check("first socket used mode=cloud and sent no extension.resume (fresh tab)",
            "mode=cloud" in socks.all[0]["url"] and all(m.get("type") != "extension.resume" for m in socks.all[0]["sent"]))

    up0 = fake.upstreams[-1]
    await fake.call_tool(up0, "update_order", MCFLURRY, "call_mcflurry")
    await fake.call_tool(up0, "update_order", COKE, "call_coke")
    ...
    rtmt.voice_choice = SERVER_VOICE  # another guest's socket changed the shared voice meanwhile
    ...
    c.check("session_metadata NOT sent on the resumed socket", not socks.received(n_socks, "extension.session_metadata"))
    ...
    c.check("'Reconnected — your order is still here.' notice shown",
            await shows(page, "Reconnected — your order is still here.", 3000))
    ...
    rotated = await page.evaluate("sessionStorage.getItem('mcdonalds.resumeId')")
    c.check("rotated resume_id stored", rotated == resumed[0]["resume_id"] and rotated != first_id)
    sent_types = [m.get("type") for m in socks.all[n_socks]["sent"] if m.get("type") != "input_audio_buffer.append"]
    c.check("voice re-sent before the browser's session.update",
            "extension.set_voice" in sent_types and "session.update" in sent_types
            and sent_types.index("extension.set_voice") < sent_types.index("session.update"), str(sent_types[:5]))
    await until(lambda: GUEST_VOICE in voices_sent(model_frames(conn1)), 5, "guest voice on the resumed upstream")
    c.check("resumed upstream is switched to the guest's voice", GUEST_VOICE in voices_sent(model_frames(conn1)),
            str(voices_sent(model_frames(conn1))))
    ...
    c.check("upstream #2 frame 2 = ONE system rehydration item containing the order",
            ...
            and "McFlurry" in item_text(frames[1]),
    ...
    c.check("bootstrap session.update precedes the nudge",
            any(f.get("type") == "session.update" for f in frames[:nudge_idx]))
    ...
    # ── 1b. the tool-error seam on the resumed socket ────────────────────────
    up1 = fake.upstreams[-1]
    n_socks_now = len(socks.all)
    await fake.call_tool(up1, "boom", {}, "call_boom")
    await until(lambda: any(TOOL_ERROR_MARKER in item_text(f) for f in model_frames(conn1)), 5, "tool-error output")
    outputs = [f for f in model_frames(conn1) if (f.get("item") or {}).get("type") == "function_call_output"]
    c.check("resumed socket: a raising tool sends an apology function_call_output to the model",
            any(o["item"].get("call_id") == "call_boom" and TOOL_ERROR_MARKER in item_text(o) for o in outputs))
    await fake.call_tool(up1, "update_order", PIE, "call_pie")
    await until(lambda: on_ticket(page, *TICKET, "Apple Pie"), 5, "third item after the tool error")
    after_boom = [i["display"] for i in json.loads(order_state_singleton.get_order_summary_json(sid))["items"]]
    c.check("resumed socket: after a tool error the next call lands on the resumed order, socket still open",
            after_boom == [*TICKET, "Apple Pie"] and len(socks.all) == n_socks_now and not socks.all[-1]["closed"]
            and next(iter(sm._attached), None) == sid, str(after_boom))
    ticket = (*TICKET, "Apple Pie")
    ...
    conn2 = conns[-1]
    c.check("mic refused -> 'Tap the mic to continue' fallback",
            await shows(page, "Reconnected — your order is still here. Tap the mic to continue."))
    ...
    c.check("no greeting after the tap", not any(GREETING_MARKER in item_text(f).lower() for f in model_frames(conn2))
            and not unprompted_creates(model_frames(conn2)))
    ...
    c.check("early tap: no 'not connected' toast", await page.get_by_text("not connected", exact=False).count() == 0)
    await until(lambda: socks.received(n_socks, "extension.session_resumed"), 15, "session_resumed after an early tap")
    conn2b = conns[-1]
    sent2b = [m.get("type") for m in socks.all[n_socks]["sent"]]
    c.check("early tap: extension.resume still the literal first frame, the held frames after it",
            tapped_early and sent2b[:1] == ["extension.resume"] and "session.update" in sent2b[1:4],
    ...
    c.check("early tap: same session, ticket kept, no greeting",
            next(iter(sm._attached), None) == sid and await on_ticket(page, *ticket)
            and not any(GREETING_MARKER in item_text(f).lower() for f in model_frames(conn2b))
            and not unprompted_creates(model_frames(conn2b)))
    ...
    c.check("reload: same session, no greeting",
            ...
            and not unprompted_creates(model_frames(conn3)))
...
async def fresh_order(c: Checks, browser, url, fake, sm, label: str):
    context, page = await new_page(browser)
    socks = Sockets(page)
    conns = fake.connections
    base = len(conns)
    await page.goto(url)
    await until(lambda: len(conns) > base and len(socks.all) == 1, 10, f"{label}: first upstream")
    await tap_mic(page, "Start recording")
    await until(lambda: appends(conns[-1]) > 0, 15, f"{label}: mic audio")
    sid = next(iter(sm._attached))
    await fake.call_tool(fake.upstreams[-1], "update_order", MCFLURRY, f"call_{label}_mcflurry")
    await until(lambda: on_ticket(page, TICKET[0]), 5, f"{label}: ticket")
    return context, page, socks, conns, sid


async def scenario_idle(c: Checks, browser, url, fake, sm):
    print("\n== 4: idle close 4000 ==", flush=True)
    context, page, socks, conns, sid = await fresh_order(c, browser, url, fake, sm, "idle")
    ...
    c.check("idle: 'Session ended after inactivity' notice",
            await shows(page, "Session ended after inactivity. Tap the mic to start a new order."))
    await asyncio.sleep(3.0)
    c.check("idle: no auto-reconnect", len(socks.all) == 1, f"{len(socks.all)} sockets")
    c.check("idle: resume id cleared", await page.evaluate("sessionStorage.getItem('mcdonalds.resumeId')") is None)
    ...
    c.check("idle: new session id", next(iter(sm._attached), None) != sid)
    c.check("idle: ticket empty", await page.get_by_text(TICKET[0], exact=True).count() == 0)
...
async def scenario_new_order(c: Checks, browser, url, fake, sm):
    print("\n== 4b: Start a new order ==", flush=True)
    context, page, socks, conns, sid = await fresh_order(c, browser, url, fake, sm, "neworder")
    old_id = await page.evaluate("sessionStorage.getItem('mcdonalds.resumeId')")
    base = len(conns)
    await page.get_by_role("button", name="Start a new order", exact=True).click()
    await until(lambda: len(socks.all) == 2, 10, "fresh socket after Start a new order")
    await until(lambda: socks.received(1, "extension.session_metadata"), 10, "fresh session_metadata")
    c.check("new order: end_session sent on the old socket",
            any(m.get("type") == "extension.end_session" for m in socks.all[0]["sent"]))
    c.check("new order: old order ended on the server", sid not in order_state_singleton.sessions)
    c.check("new order: the fresh socket does not resume",
            all(m.get("type") != "extension.resume" for m in socks.all[1]["sent"]))
    new_id = await until(lambda: page.evaluate("sessionStorage.getItem('mcdonalds.resumeId')"), 5, "new resume id")
    c.check("new order: a new resume id replaces the old one", bool(new_id) and new_id != old_id)
    c.check("new order: ticket empty and the button gone",
            await page.get_by_text(TICKET[0], exact=True).count() == 0
            and await page.get_by_role("button", name="Start a new order", exact=True).count() == 0)
    await tap_mic(page, "Start recording")
    await until(lambda: len(conns) > base and any(f.get("type") == "response.create" for f in model_frames(conns[-1])),
                10, "greeting on the fresh session")
    c.check("new order: the next tap greets a fresh session", next(iter(sm._attached), None) != sid)
    await context.close()
...
async def scenario_strict_autoplay(c: Checks, p, launch: dict, url, fake, sm):
    ...
    try:
        ...
        sid = next(iter(sm._attached))
        await fake.call_tool(fake.upstreams[-1], "update_order", MCFLURRY, "call_strict_mcflurry")
...
async def main() -> int:
    ...
    try:
        async with async_playwright() as p:
            ...
            try:
                for run in (lambda: scenario_resume(c, browser, url, fake, rtmt, sm, logs),
                            lambda: scenario_idle(c, browser, url, fake, sm),
                            lambda: scenario_new_order(c, browser, url, fake, sm),
