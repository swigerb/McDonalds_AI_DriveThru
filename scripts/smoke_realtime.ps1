...
# Skip it with:  azd env set MCD_SKIP_REALTIME_SMOKE true
$ErrorActionPreference = "Continue"

if ($env:MCD_SKIP_REALTIME_SMOKE -eq "true") {
  Write-Host "Realtime smoke check skipped (MCD_SKIP_REALTIME_SMOKE=true)."
  exit 0
}
...
if ($code -ne 0) {
  ...
  if ($code -eq 1) {
    ...
    Write-Warning " the crew member's session config. Tools may NOT register. See above."
  } else {
