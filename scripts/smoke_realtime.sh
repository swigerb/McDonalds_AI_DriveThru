...
if [ "$MCD_SKIP_REALTIME_SMOKE" = "true" ]; then
  echo "Realtime smoke check skipped (MCD_SKIP_REALTIME_SMOKE=true)."
  exit 0
fi
...
if [ "$code" -ne 0 ]; then
  ...
  if [ "$code" -eq 1 ]; then
    ...
    echo "WARNING:  the crew member's session config. Tools may NOT register. See above."
  else
